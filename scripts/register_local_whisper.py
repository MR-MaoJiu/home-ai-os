"""为指定成员登记已运行的本地 whisper.cpp；服务凭据只写入加密 Secret。"""
import argparse
import asyncio
...
async def main():
    ...
    token=Path('state/provider-secrets/whisper.token').read_text().strip()
    async with httpx.AsyncClient(trust_env=False,timeout=10) as client:
        response=await client.get('http://127.0.0.1:8105/health',headers={'Authorization':'Bearer '+token})
        response.raise_for_status()
        if response.json().get('adapter')!='whisper':raise SystemExit('端口不是 whisper.cpp 服务')
    ...
    with factory() as db:
        ...
        provider_id='whisper.'+principal.id
        secret_id=uid()
        ...
        manifest=ProviderManifest(id=provider_id,version='1.9.3',adapter='http',endpoint='http://127.0.0.1:8105',capabilities={'speech.transcribe@v1':'/invoke/transcribe'},allowed_hosts=['127.0.0.1'],secret_id=secret_id,timeout_seconds=300)
        ...
        db.commit()
    print('whisper.cpp 已为指定成员登记，服务凭据不会回显。')
