"""为指定成员登记已运行的本地 Graphiti；服务凭据只写入加密 Secret。"""
import argparse
import asyncio
...
async def main():
    ...
    token=Path('state/provider-secrets/graphiti.token').read_text().strip()
    async with httpx.AsyncClient(trust_env=False,timeout=10) as client:
        response=await client.get('http://127.0.0.1:8102/health',headers={'Authorization':'Bearer '+token})
        response.raise_for_status()
        if response.json().get('adapter')!='graphiti':raise SystemExit('端口不是 Graphiti 服务')
    ...
    with factory() as db:
        ...
        provider_id='graphiti.'+principal.id
        secret_id=uid()
        ...
        manifest=ProviderManifest(id=provider_id,version='0.30.2',adapter='http',endpoint='http://127.0.0.1:8102',capabilities={'memory.graph.search@v1':'/invoke/search','memory.graph.index@v1':'/invoke/index','memory.graph.purge@v1':'/invoke/purge'},allowed_hosts=['127.0.0.1'],secret_id=secret_id,timeout_seconds=300)
        ...
        db.commit()
    print('Graphiti 已为指定成员登记，服务凭据不会回显。')
