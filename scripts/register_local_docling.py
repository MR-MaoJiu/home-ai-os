"""为指定成员登记已运行的本地 Docling；服务凭据只写入加密 Secret。"""
import argparse
import asyncio
...
async def main():
    ...
    async with httpx.AsyncClient(trust_env=False,timeout=10) as client:
        response=await client.get('http://127.0.0.1:8103/health',headers={'Authorization':'Bearer '+token})
        response.raise_for_status()
        if response.json().get('adapter')!='docling':raise SystemExit('端口不是 Docling 服务')
    ...
    with factory() as db:
        ...
        provider_id='docling.'+principal.id
        secret_id=uid()
        ...
        db.commit()
    print('Docling 已为指定成员登记，服务凭据不会回显。')
