"""真实 FunASR 转写；英文人声和中文合成音频仅作测试输入。"""
...
pytestmark=pytest.mark.skipif(os.environ.get('HOMEAI_FUNASR_TEST')!='1',reason='需要真实 FunASR、模型、语音桥、PostgreSQL、OPA 和音频文件')


@pytest.mark.asyncio
@pytest.mark.parametrize('filename,language,expected',[
    ('state/whisper.cpp/samples/jfk.wav','en',['ask not','country']),
    ('state/voice-fixtures/chinese.wav','zh',['明天','3点','备份']),
    ('state/voice-fixtures/chinese.wav','auto',['明天','3点','备份']),
...
async def test_real_transcription(filename,language,expected):
    ...
    provider_id='a.funasr.live';secret_id=uid();token=Path('state/provider-secrets/funasr.token').read_text().strip()
    with app.state.db() as db:
        ...
        manifest=ProviderManifest(id=provider_id,version='1.4.14',adapter='http',endpoint='http://127.0.0.1:8104',allowed_hosts=['127.0.0.1'],secret_id=secret_id,timeout_seconds=180,capabilities={'speech.transcribe@v1':'/invoke/transcribe'})
    ...
    try:
        ...
        assert task['result']['provider']=='funasr'
        ...
        async with httpx.AsyncClient(trust_env=False,headers={'Authorization':'Bearer '+token}) as client:
            health=(await client.get('http://127.0.0.1:8104/health')).json()
