...
def accessible(db, actor, include_deleted=False):
    ...
    query = select(Record).where(Record.household_id == actor.household_id, or_(Record.owner_id == actor.user_id, Record.id.in_(granted)))
...
def ingest(db, actor, item, vault):
    ...
    record.sensitivity = "SENSITIVE" if sensitive else item.sensitivity
