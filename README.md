...
用户要求平台只协助连接，业务数据不能经过平台或第三方中继。历史版本采用 `HTTPS → HK HAProxy/frps → 家庭 frpc`，虽然家庭端终止 TLS，但全部业务密文仍经过 HK；**那不是 P2P**。历史连通、限流和计费验收不能证明纯直连完成。

当前源码已关闭旧中继绑定、恢复及启动入口；纯直连协议、移动端集成和跨网络验证尚未完成，不能将此版本称为已实现 P2P。闭源平台的旧套餐/部署需要另行迁移，其带宽与流量字段不适用于新的业务模型。升级代码不会终止另一台机器或本机仍运行的旧进程，管理员必须停止旧版连接程序后再部署新版本。
...
本仓库提供家庭服务端、iOS 客户端、网页管理后台和独立 Provider 适配器。Home AI Connect 的账号、套餐、订单、运营后台属于独立闭源项目（历史中继计量待移除），不在本仓库；家庭私人数据不上传到平台数据库。
...
| 远程 | 稳定服务器身份、换证书/地址验证；旧中继入口已停用 | 纯直连信令、打洞、iOS 接入、无中继跨网络验收；闭源平台套餐迁移 |
...
以下为正在实施的目标架构，**不是当前已通过验收的功能**。

```mermaid
flowchart LR
    PHONE[iOS 客户端] -. 身份验证 / 信令 .-> PLATFORM[独立 Home AI Connect]
    HOME[家庭服务端] -. 身份验证 / 信令 .-> PLATFORM
    PHONE <-->|端到端加密业务数据 / 仅直连| HOME
```

1. 家庭服务生成稳定身份，手机通过本机配对建立对家庭身份的信任；平台会话不授予家庭数据权限。
2. 平台验证账号与实例授权，交换建立连接所需的有限信息，不接受家庭正文、文件、语音作为信令。
3. 两端尝试可直达地址与 NAT 打洞；平台只协调。所有候选路径必须排除中继，禁止 TURN/STCP/HTTPS 中继回退。
4. 两端连接后核验已配对家庭身份，才发送认证凭据与业务请求。域名证书或平台签发的令牌不能替换已配对身份。
5. 直连失败明确显示原因；切换网络后重新协商。部分 NAT/防火墙条件下可能无法连接，不能承诺任意网络可用。
6. 平台不可用不影响本地功能；公网传输速度取决于两端网络。平台可以按实例数和服务期限提供订阅，但不再按业务转发流量收费。
...
| 远程证书 | 正式 CA/DNS 验收及定时续期、过期失败处理、长期断网与配额恢复 |
...
已接入[官方 ACME 客户端](https://acme-python.readthedocs.io/en/stable/api/client.html) `acme 5.8.0`，可为当前绑定实例申请证书并暂存结果。家庭端生成并持有 ACME 账户私钥和 TLS 私钥；Cloudflare Token 仍只在独立连接平台中。受管 HTTPS 安装与热切换已通过本机真实 TLS 验收；**正式公网部署和定时续期仍未完成**。申请命令仍只产生暂存结果，安装流程见下一节。
...
目前没有自动核对未知新订单的运营界面。`state/acme/`、TLS 证书包、选择状态和加密远程绑定已纳入备份/隔离恢复范围；实际 TLS 与身份文件恢复已验证，完整 ACME 断点的异机恢复仍待验收。
...
远程受管入口默认监听本机 `58448`，原本地入口 `58443` 保留。两者共享 Core 数据库、主密钥与稳定服务器身份；远程证书、套餐或隧道失效不会关闭本地入口。不要用远程受管进程替换仍需独立运行的本地服务。

### 安装与启动

取得正式 ACME 证书后，选择暂存目录：

```sh
.venv/bin/python scripts/activate_certificate.py --bundle state/acme/实例域名/生成标识
.venv/bin/python scripts/run_managed_api.py
```

选择操作验证系统信任根、证书链、域名、有效期、私钥匹配及文件权限；测试证书不会被默认接受。验证通过后复制成 `state/tls/bundles/<标识>/` 内的独立快照，并原子写入加密选择记录。返回 `selected` 只表示选中，不能当作已提供服务。

受管进程读取选择记录，每两秒检查变化，建立新的完整 SSLContext 后切换。失败保持原上下文；进程重启时如候选损坏，会尝试恢复最近仍有效的运行证书。服务身份响应读取实际加载的叶证书，避免文件已变化但服务仍提供旧证书时错误声明。过期证书拒绝新握手，已经建立的 HTTP 连接也不能继续发起有效业务请求。

```sh
# 查看近期实际运行状态；超过十秒的状态不会继续报告正在监听。
.venv/bin/python scripts/activate_certificate.py --status

# 重新选择仍有效且校验通过的历史快照。
.venv/bin/python scripts/activate_certificate.py --rollback 历史证书包标识
```

快照保留独立密钥与选择记录，回滚不依赖原暂存私钥仍然存在。历史证书过期或损坏时不能回滚。仅本机开发验收可显式提供 `--test-ca`；生产模式拒绝该参数对应的测试信任。

### 连接平台与旧绑定迁移

`HOMEAI_MANAGED_HTTPS_PORT` 默认 `58448`，本地配置控制独立受管 HTTPS 端口。证书加载、热切换与回滚工具保留，但受管 HTTPS 就绪只证明本机 TLS 入口正常，不证明公网可达或 P2P 成功。

纯直连架构变更后，旧中继绑定/迁移/恢复入口与 frpc 启动路径已停用。前述 TLS 实验保留为证书组件的历史验证，不能作为新直连传输的验收证据。


### 备份与验证范围

备份范围增加 ACME 账户/订单/暂存文件、TLS 文件与快照、选择/运行状态及加密远程绑定；证书操作锁被占用时拒绝开始备份。恢复只写入隔离目录，主密钥仍需另行保管，不直接覆盖运行服务。

本轮真实 CA/HTTPS 验收通过：证书换钥热切换、无 SNI 连接、证书与身份声明一致、损坏候选保留旧证书、进程重启回退，以及原暂存私钥删除后的快照回滚。管理页经真实浏览器验证状态和配置端口。维护窗口备份恢复后，现有 TLS 私钥、证书和服务器身份字节及权限一致。

正式公网证书、无人值守续期、生产部署与完整 ACME 异机恢复仍待验收；本机当前没有启用正式官方远程绑定。
