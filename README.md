...
| 远程 | 主动 frp 隧道、实例签名、租约、TLS 透传；稳定服务器身份、换证书/地址验证；已有真实连通/撤销记录 | 正式 DNS/CA 签发与定时续期、长期断网与配额故障演练 |
...
`HOMEAI_MANAGED_HTTPS_PORT` 默认 `58448`，本地配置控制端口，网页绑定表单不能任意指定其他服务端口。新绑定使用该受管端口。已有绑定不会静默改变；在管理后台“远程连接”中，待正式证书、域名、端口和运行状态匹配后，再点击“将已有绑定切换到就绪的受管 HTTPS”。浏览器执行此操作需要近期重新认证。

远程连接程序只有在匹配域名的正式受管 HTTPS 就绪后才申请隧道；测试证书、失效状态或不匹配端口进入 `waiting_certificate`。端口、凭据或绑定配置变化会重建 frpc；旧进程停止有超时兜底。暂停后的连接可显式恢复，但仍需重新满足证书与租约检查。升级旧连接程序前应先准备受管入口并迁移，否则程序会等待证书而不建立隧道。
