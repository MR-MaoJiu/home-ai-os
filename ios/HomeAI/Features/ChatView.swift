...
struct ChatView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                    ...
                    LazyVStack(alignment: .leading, spacing: 16) {
                        ForEach(messages) { message in
                            HStack {
                                ...
                                Text(message.text).textSelection(.enabled).padding().background(message.mine ? Color.teal.opacity(0.12) : Color.secondary.opacity(0.08), in: RoundedRectangle(cornerRadius: 18))
                                if !message.mine { Spacer(minLength: 32) }
                            }
                        }
                    }.padding()
            ...
            HStack(alignment: .bottom) {
                Button {
                    Task { await state.perform {
                        if voice.recording {
                            ...
                            let data = try await state.api.request("POST", "/api/v1/tasks", body: body)
                            let task = try JSONDecoder().decode(TaskCreated.self, from: data)
                            for _ in 0..<120 {
                                let raw = try await state.api.request("GET", "/api/v1/tasks/" + task.id)
                ...
                .disabled(!state.connected || sending || state.busy)
                ...
                Button { Task { await send() } } label: { Image(systemName: "arrow.up.circle.fill").font(.largeTitle) }
                    .disabled(sending || input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || !state.connected)
                    .accessibilityLabel("发送")
            }.padding()
        }
        .navigationTitle("Home AI")
        .toolbar { Text(state.connected ? "家庭服务器" : "未配对").font(.caption).foregroundStyle(.secondary) }
    }
    ...
    private func send() async {
        ...
        await state.perform {
            ...
            for _ in 0..<300 {
                ...
                let result = try await state.api.request("GET", "/api/v1/tasks/" + created.id)
                ...
                if ["SUCCEEDED", "FAILED", "CANCELED", "NEEDS_RECONCILIATION", "AWAITING_APPROVAL"].contains(task.status) {
                    messages.append(ChatMessage(text: task.error ?? Self.answer(task.result) ?? (task.status == "AWAITING_APPROVAL" ? "请在活动页面确认操作。" : task.status), mine: false))
                    return
                }
                try await Task.sleep(for: .seconds(1))
            }
    ...
    private func cancel() async {
        ...
        await state.perform { _ = try await state.api.request("POST", "/api/v1/tasks/" + currentTask + "/cancel") }
    }
