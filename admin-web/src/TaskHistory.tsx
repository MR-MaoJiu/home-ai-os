...
export function TaskHistory({tasks,onChanged}:{tasks:Task[]|null;onChanged:()=>void}){
 ...
 return <section><h2>最近任务</h2>{error&&<p className="message error" role="alert">{error}</p>}
  ...
  {selected&&<div><h3>任务 {selected.id.slice(0,8)} · {selected.status}</h3>{selected.error&&<p className="message error">{selected.error}</p>}<p>已完成步骤会保留；取消和重启不会自动撤销已经发生的外部操作。</p>
   ...
   {steps.map(step=><details key={step.id}><summary>步骤 {step.step+1} · {step.capability} · {step.status==='SKIPPED'?'条件不满足，已跳过':step.status}</summary>{step.capability==='mail.send@v1'&&step.status==='NEEDS_RECONCILIATION'&&<p>邮件发送结果需要本机核对。调用 ID：<code>{step.id}</code>。先使用 mail_delivery.py 检查并记录结论；确认未执行后，还需在此重新核对并审批。</p>}{step.status==='SKIPPED'?<p>此步骤没有调用工具，也没有生成执行结果。</p>:<pre>{JSON.stringify(step.result,null,2)}</pre>}</details>)}
