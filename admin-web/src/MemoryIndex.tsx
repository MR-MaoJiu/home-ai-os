...
export function MemoryIndex({revision,onChanged}:{revision:number;onChanged:()=>void}){
 ...
 async function rebuildDerived(provider:string){
  if(!confirm('清除并重建当前账户的这个派生索引？核心记忆不会删除。'))return;
  setBusy(true);setError('');
  try{await api('/memory/derived/'+encodeURIComponent(provider)+'/rebuild','POST');onChanged()}
 ...
 return <section><div className="section-title"><h2>记忆索引</h2><button onClick={onChanged}>刷新状态</button></div>
  ...
  {derived.length>0&&<><h3>外部派生索引</h3>{derived.map(row=><div className="row" key={row.provider_id}><b>{row.provider_id}</b><span>{labels[row.status]??row.status}</span><span>累计重建 {row.attempts} 次</span>{row.error_type&&<span>{row.error_type}</span>}<button disabled={busy} onClick={()=>rebuildDerived(row.provider_id)}>重建我的投影</button></div>)}</>}
 </section>
}
