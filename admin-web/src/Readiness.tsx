...
import {api} from './api';
type Report={core_dependencies_ready:boolean;checked_at:number;checks:Record<string,{ok:boolean;reason:string}>;unverified:string[]};
const names:Record<string,string>={database:'数据库与权限隔离',policy:'策略决策',events:'事件服务',encryption:'加密服务'};
export function Readiness(){
 ...
 return <section><h2>部署检查</h2><p>只读检查基础依赖。通过不代表模型、worker、生产隔离或家庭部署已经验收。</p>
  ...
  {report&&<><p role="status">{report.core_dependencies_ready?'基础依赖检查通过':'部分基础依赖未就绪'} · {new Date(report.checked_at*1000).toLocaleString()}</p>
